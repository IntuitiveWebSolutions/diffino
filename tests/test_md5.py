...
def test_single_file_csv_s3_md5():
    diff = Diffino(
        mode="md5", left="s3://fake-bucket/one.csv", right="s3://fake-bucket/two.csv"
    )
    results = diff.build_diff()
    assert results
...
def test_multiple_files_s3_md5():
    diff = Diffino(
        mode="md5", left="s3://fake-bucket/one", right="s3://fake-bucket/two"
    )
    results = diff.build_diff()
    assert results
...
def test_output_csv_md5():
    diff = Diffino(
        mode="md5", left="/tmp/one.csv", right="/tmp/two.csv", output="/tmp/diff.csv"
    )
    diff.build_diff()
...
def test_output_xlsx_md5():
    diff = Diffino(
        mode="md5", left="/tmp/one.csv", right="/tmp/two.csv", output="/tmp/diff.xslx"
    )
    diff.build_diff()
    raise Exception("Finish test!")
