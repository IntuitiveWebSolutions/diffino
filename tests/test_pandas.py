...
def test_single_file_csv_s3_pandas():
    diff = Diffino(
        mode="pandas", left="s3://fake-bucket/one.csv", right="s3://fake-bucket/two.csv"
    )
    results = diff.build_diff()
    assert results
...
def test_multiple_files_s3_pandas():
    diff = Diffino(
        mode="pandas", left="s3://fake-bucket/one", right="s3://fake-bucket/two"
    )
    results = diff.build_diff()
    assert results
...
def test_specific_cols():
    diff = Diffino(
        mode="pandas", left="/tmp/one.csv", right="/tmp/two.csv", cols=["id", "name"]
    )
    results = diff.build_diff()
    assert results
...
def test_output_csv_pandas():
    diff = Diffino(
        mode="pandas", left="/tmp/one.csv", right="/tmp/two.csv", output="/tmp/diff.csv"
    )
    diff.build_diff()
